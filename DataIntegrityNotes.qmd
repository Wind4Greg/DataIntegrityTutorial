...
Added redirects for the context and vocabulary files via nginx rewrite directives in the `/etc/nginx/sites-available/bawfc.grotto-networking.com` file. Also had to add CORS line `add_header 'Access-Control-Allow-Origin' '*';` so the JSON-LD playground could get the file.
